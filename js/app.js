...
// =========================================================================
// 1. IMPORTACIONES DE FIREBASE (Versión 11.9.1)
// =========================================================================
...
// =========================================================================
// 2. NAVEGACIÓN Y LÓGICA DE VISTAS
// =========================================================================
window.switchView = function(id) {
  ...
  // Carga dinámica al cambiar de vista
  if (id === "galeria") buildGallery();
  if (id === "resenas") buildReviews();
};
...
// =========================================================================
// 3. FUNCIONALIDADES DE UI (Galería, Servicios, Social)
// =========================================================================
...
async function loadServices() {
  ...
  snap.forEach(docSnap => {
    ...
    card.innerHTML = `<div class="srv-img"><img src="${s.image}" alt="${s.name}"></div><h3>${s.name}</h3><p>${s.description}</p><span class="srv-price">$${s.price}</span>`;
    grid.appendChild(card);
  });
}

// =========================================================================
// 4. ENVÍO DE FORMULARIOS A FIREBASE (Citas y Reseñas)
// =========================================================================
...
document.getElementById("btn-enviar-cita")?.addEventListener("click", async () => {
  ...
  if (!name || !phone || !service || !date) { alert("Completa todos los campos obligatorios"); return; }

  try {
    await addDoc(collection(db, "appointments"), {
      name, phone, service, date, message,
      status: "Pendiente",
      creadoEn: serverTimestamp() // Clave para la notificación en admin
    });
    alert("Solicitud enviada correctamente ✅");
    // Limpiar formulario
    document.querySelectorAll("#view-citas input, #view-citas textarea").forEach(i => i.value = "");
...
document.getElementById("btn-enviar-resena")?.addEventListener("click", async () => {
  ...
  if (!selectedStars || !name || !comment) { alert("Por favor completa todos los campos y califica"); return; }
  ...
  try {
    ...
    document.querySelectorAll("#view-resenas input, #view-resenas textarea, #view-resenas select").forEach(i => i.value = "");
